  ...
  for i,list in pairs(notAcquiredItems) do
    ...
    for j=1,#list do
      if (#list[j] + #str[#str]) > 250 then
        str[#str + 1] = bossName..L[": "]..list[j]
      else
        if j == 1 then
          str[#str] = str[#str]..L[": "]..list[j]
        else
      ...
      if item then
        ...
        if bosses or item.misc or item.customitem then
          ...
          if count(specFilter) ~= 1 then
            text = (" (%s)"):format(tconcat(specs, "/"))
          end
          if hasItemOnHigherDifficulty then
            text = (" - %s"):format((L["You have this item on '%s'"]):format(self:GetDescription(self.DIFFICULTY, selectedDungeon, hasItem)))
          elseif hasItem then
            text = (" - %s"):format(L["You have this item"])
          end
          label:SetText(RETRIEVING_ITEM_INFO..text)
          local itemInfo = Item:CreateFromItemID(id)
          itemInfo:ContinueOnItemLoad(function() label:SetText(itemInfo:GetItemLink()..text) end)
          if firstLabel then
            if bosses then
...
function Overview:UpdateContent()
  ...
  if #bossLootGroups == 0 and not misc then
    ...
    hasContent = false
   --TWW reportButton:SetDisabled(true)
