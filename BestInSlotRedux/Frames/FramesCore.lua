...
=     GetSpecializationInfoByID, C_Item.GetItemStatDelta, IsControlKeyDown, IsShiftKeyDown, SetCursor, ResetCursor, DressUpItemLink, C_Item.GetItemIconByID, C_AddOns.GetAddOnMetadata, GetSpecializationInfo, PlaySound, GetNumSpecializations, UnitAffectingCombat
...
local function addGuildInfoToTooltip(tooltip, itemId, difficultyId)
  if BestInSlot.db.char.options.guildtooltip then
    ...
    for player, playerSpecs in pairs(players) do
      ...
      if not guildRank then guildRank = 4 end
      if guildRank and BestInSlot.db.char.options.showGuildRankInTooltip[guildRank] then
        ...
        for specId, obtained in pairs(playerSpecs) do
          local tbl = specNames
          local specialText = ""
          if type(obtained) == "table" then
            if obtained.text ~= L["Token"] then
              specialText = " ("..obtained.text..")"
            end
            obtained = obtained.obtained
          end
          if obtained then tbl = specNamesObtained end
          local specName = select(2, GetSpecializationInfoByID(specId))
          tinsert(tbl, specName..specialText)
        end
        if #specNames > 0 then
          if first then
...
function BestInSlot:GetItemTooltip(itemid, difficulty, frame, itemlink)
  if frame then
    GameTooltip:SetOwner(frame.frame, "ANCHOR_NONE")
  ...
  GameTooltip:SetHyperlink(itemlink or item.link, playerClass, select(2, self:GetSelected(self.SPECIALIZATION)))
...
--	if self.db.char.options.showBiSTooltip and (self.db.char.options.tooltipCombat or not UnitAffectingCombat("player")) then
    ...
    if link then
      local itemId, instanceID = BestInSlot:GetItemInfoFromLink(link)
      local exists, existType, dungeon = BestInSlot:ItemExists(itemId)
      if not exists then return end
      if itemId then
        --instanceID = tonumber(instanceID)
        --instanceID = BestInSlot:GetDifficultyIdForDungeon(instanceID, nil, true) or 1
        local bisInfo
        if existType == "item" then
          bisInfo = BestInSlot:IsItemBestInSlot(itemId, instanceID)
        else
          bisInfo = BestInSlot:IsTokenBestInSlot(itemId, instanceID)
        end
        ...
        if bisInfo and type(bisInfo) == "table" then
          ...
          local first = true
          local hasItems
          for specId, itemId in pairs(bisInfo) do
            if first then
              isBiS = true
            end
            isBiS = isBiS and itemId
            if not first then
              specsText = specsText.."/"
            end
            local specialText = ""
            ...
            if item.itemid ~= itemId then
              local itemType = select(2, BestInSlot:IsItemTierToken(item.itemid))
              if itemType ~= L["Token"] then
                specialText = (" (%s)"):format(itemType)
              end
            end
            specsText = specsText..(type(specId) == "string" and specId or select(2,GetSpecializationInfoByID(specId)))..specialText --custom lists can just show as custom list, otherwise lookup spec name
            first = false
            if type(itemId) == "table" then
              hasItems = true
              for _, tierId in pairs(itemId) do
                hasItems = hasItems and BestInSlot:HasItem(tierId, instanceID, true)
              end
            end
          end
          if isBiS then
            local color = BestInSlot.colorHighlight
            if hasItems or type(isBiS) == "number" and BestInSlot:HasItem(isBiS, instanceID, true) then
              color = GREEN_FONT_COLOR_CODE
            end
            tooltip:AddLine(("%1$s (%2$s)"):format(color.."BestInSlot"..BestInSlot.colorNormal, color..specsText.."|r"))
          end
        end
        if item and (BestInSlot.db.char.options.tooltipSource or BestInSlot.options.DEBUG) then
          if item.multiplesources then
            tooltip:AddLine(L["SOURCES"]..":")
            local bosslist = {}
            for dungeonname,v in pairs(item.multiplesources) do
              for bossid in pairs(v) do
                if not tContains(bosslist, BestInSlot:GetDescription(BestInSlot.BOSS, dungeonname, bossid)..BestInSlot:GetDescription(BestInSlot.INSTANCE, dungeonname)) then
                  tooltip:AddLine(("%s%s %s(%s)|r"):format(BestInSlot.colorHighlight, BestInSlot:GetDescription(BestInSlot.BOSS, dungeonname, bossid), BestInSlot.colorNormal, BestInSlot:GetDescription(BestInSlot.INSTANCE, dungeonname)))
                  tinsert(bosslist, BestInSlot:GetDescription(BestInSlot.BOSS, dungeonname, bossid)..BestInSlot:GetDescription(BestInSlot.INSTANCE, dungeonname))
                end
              end
            end
          elseif item.dungeon and item.bossid then
            tooltip:AddLine(("%s %s%s"):format(SOURCE, BestInSlot.colorHighlight, BestInSlot:GetDescription(BestInSlot.BOSS, item.dungeon, item.bossid)))
          elseif item.misc then
            tooltip:AddLine(("%s %s%s"):format(SOURCE, BestInSlot.colorHighlight, item.misc))
          end
        end
        if BestInSlot:ItemExists(itemId) then
...
function BestInSlot:GetItemLinkLabel(itemid, difficulty, withIcon)
  ...
  if itemid ~= "PLACEHOLDER" then
    ...
    itemInfo:ContinueOnItemLoad(function()
      item = self:GetItem(itemid, difficulty)
      label:SetUserData("itemlink", item.link)
    end)
    label:SetText(item and item.link or RETRIEVING_ITEM_INFO)
    if withIcon then
...
local function GetFrame()
  ...
  frame:SetTitle("Best In Slot Redux Manager "..GetAddOnMetadata("BestInSlotRedux", "Version"))
  ...
  for i=1,#menuItems do
    ...
    if item.IsShown() then
      ...
      interactiveLabel:SetFont(GameFontNormalSmall:GetFont(), 14, "OUTLINE")
...
function BestInSlot:ValidateSelected(type)
  if type == self.RAIDTIER then
    ...
    local isDisabled = string.find(difficulties[selected[self.DIFFICULTY]], "|cFFFFFFFF$")
    if isDisabled then
      for i = 1, #difficulties do
        if i ~= selected[self.DIFFICULTY] and string.find(difficulties[i], string.sub(difficulties[selected[self.DIFFICULTY]], 1, isDisabled - 1)) then
          self:SetSelected(self.DIFFICULTY, i)
          break
        end
      end
    end
...
local function getDifficultyDropdown(dropdown)
  ...
  local difficulties = BestInSlot:GetDifficulties(BestInSlot.RAIDTIER, raidTierValue)
  --dropdown:SetList(BestInSlot:GetDifficulties(BestInSlot.RAIDTIER, raidTierValue))
  dropdown:SetList({})
  for i = 1, #difficulties do
    dropdown:AddItem(i, difficulties[i])
    if string.find(difficulties[i], "|cFFFFFFFF$") then
...
local function getDungeonDropdown(dropdown)
  ...
  for i=1,#instances do
    local expansionName = BestInSlot:GetDescription(BestInSlot.EXPANSION, BestInSlot:GetExpansions(BestInSlot.INSTANCE, instances[i]))
    local tier = BestInSlot:GetDescription(BestInSlot.RAIDTIER, BestInSlot:GetRaidTiers(BestInSlot.INSTANCE, instances[i]))
    if string.find(tier, "|cFFFFFFFF$") then
      expansionName = expansionName.." "..tier
    end
    if expansionName ~= lastExpansionName then
      dropdown:AddItem(expansionName, "    "..expansionName)
      dropdown:SetItemDisabled(expansionName, true)
