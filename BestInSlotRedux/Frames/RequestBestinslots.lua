...
function RequestBiS:SetText(label)
  ...
  if not itemLink then
    local itemInfo = Item:CreateFromItemID(label:GetUserData("itemid"))
    itemInfo:ContinueOnItemLoad(function() RequestBiS:SetText(label) end)
  end
  local chars = label:GetUserData("chars")
  local str = itemLink or RETRIEVING_ITEM_INFO
  local first = true
...
function RequestBiS:RegisterRaidTier(name, bis, specId)
  ...
  for i,_ in pairs(bis) do
    ...
    if showObtainedItems or not obtained then
      ...
      if not customitem and item and (item.multiplesources and item.multiplesources[responseContainer:GetUserData("selection")] or item.dungeon == responseContainer:GetUserData("selection")) then --item should be from this instance
...
local function requestRaidTierReply(itemlist, channel, source, version)
  ...
  --elseif responseChannel == "WHISPER" then
    ...
    --if #list ~= 0 and list[source] then
      --RequestBiS:SaveGuildBestInSlotList(guildName, source, itemlist, selected.raidtier, selected.difficulty, nil, version)
    --end
  --end
  RequestBiS:Print(itemlist)
