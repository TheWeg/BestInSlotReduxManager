...
=     GetUnitName, C_AddOns.GetAddOnMetadata, IsInGuild, IsInGroup
  ...
  if prefix == BestInSlot.MSGPREFIX and source ~= playerName then
    ...
    if success then
      ...
      if printUpdateReminder and BestInSlot.version < version and not msg.Alpha and version > 136 then
        BestInSlot:Print((L["Your version of BestInSlot is outdated! You can download the newest version at %s%s"]):format(BestInSlot.colorHighlight, "https://www.curseforge.com/wow/addons/bestinslotreduxmanager"), true)
...
local function versionCheck(data, channel, source)
  if not versionNumber then
    versionNumber = C_AddOns.GetAddOnMetadata("BestInSlotRedux", "Version")
